...
- (id)handlerForIntent:(INIntent *)intent {
    ...
    //接受到语音后，采用系统的还是自己封装的
    //在当前和UI 里 info.plist 要有相应的配置(<Intents/Intents.h>) 查看以 /...Intents.h结尾的
    NSLog(@"%s", __FUNCTION__);
    if ([intent isKindOfClass:[INSendMessageIntent class]]) {
        NSLog(@"新哥语音发送消息");
        return [[RXINSendMessageHandler alloc] init];
    }
    else if([intent isKindOfClass:[INSearchForPhotosIntent class]]) {
        NSLog(@"新哥语音搜索图片");
        return [[RXINSearchForPhotosHandler alloc] init];
    }
        ...
        NSLog(@"intent Class = %@", NSStringFromClass([intent class]));
    }
